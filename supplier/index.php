...
$where = $cari ? "WHERE kode_supplier LIKE '%$cari%' OR nama_supplier LIKE '%$cari%'" : '';
$result = mysqli_query($conn, "SELECT * FROM supplier $where ORDER BY id DESC");
?>
<div class="page-header d-flex justify-content-between align-items-center">
  <h4 class="mb-0"><i class="bi bi-truck me-2"></i>Data Supplier</h4>
        ...
        <tbody>
          ...
          if (mysqli_num_rows($result) > 0):
            while ($r = mysqli_fetch_assoc($result)): ?>
              <tr>
                <td><?= $no++ ?></td>
                <td><span class="badge bg-secondary"><?= $r['kode_supplier'] ?></span></td>
